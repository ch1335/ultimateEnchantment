...
public class Terminator extends SingleAttributeEnchantment {
    ...
    public void shoot(ItemStack pStack, Level pLevel, Player player, int pTimeLeft) {
        this.multipleShooting(pStack, pLevel, player, pTimeLeft);
        SimpleSchedule.addSchedule(pLevel, new SimpleSchedule.Wait(() -> this.SingleShoot(pStack, pLevel, player, pTimeLeft), 2));
    }

    public void SingleShoot(ItemStack pStack, Level pLevel, Player player, int pTimeLeft) {
        ItemStack oldUseItem = player.getUseItem();
        player.useItem = pStack;
        pStack.releaseUsing(pLevel, player, pTimeLeft);
        this.multipleShooting(pStack, pLevel, player, pTimeLeft);
        player.useItem = oldUseItem;
    }

    public void multipleShooting(ItemStack pStack, Level pLevel, Player player, int pTimeLeft) {

        for (int side : SIDES) {
            for (int i = 0; i < additionalShotPerSide; i++) {
                ItemStack itemstack = player.getProjectile(pStack).copy();
                itemstack.getOrCreateTag().putBoolean("ue:createByTerminator", true);
                int useTime = pStack.getUseDuration() - pTimeLeft;
                int j = ForgeEventFactory.onArrowLoose(pStack, pLevel, player, useTime, !itemstack.isEmpty());
                if (j >= 0) {
                    ...
                    abstractarrow.shootFromRotation(player, player.getXRot(), player.getYRot() + ((float) 5 / additionalShotPerSide) * side, 0.0F, f * 3.0F, 1.0F);
