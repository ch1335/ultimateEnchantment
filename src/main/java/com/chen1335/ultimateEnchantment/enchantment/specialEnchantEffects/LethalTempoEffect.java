...
public class LethalTempoEffect {
    ...
    public static void onArrowHit(LivingIncomingDamageEvent event) {
        if (event.getSource().getDirectEntity() instanceof Projectile projectile && projectile.getOwner() instanceof LivingEntity living) {
            ...
            Holder.Reference<Enchantment> holder = living.level().registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(UEEnchantments.LETHAL_TEMPO);

            int lethalTempoLevel = weapon.getEnchantmentLevel(holder);
            LethalTempoComponent lethalTempoComponent = Objects.requireNonNull(holder.value().effects().get(UEEnchantmentEffectComponents.LETHAL_TEMPO.get()));
            if (lethalTempoLevel <= 0) {
                return;
            }
            ...
            if (!projectileMixin.ue$isLethalTempoAdditionArrow()) {
                ...
                weapon.set(UEDataComponentTypes.ADDITION_SHOOT_CHANCE, Math.min(lethalTempoComponent.maxChancePerLevel() * lethalTempoLevel, oldChance + lethalTempoComponent.addChanceOnHit()));
                weapon.set(UEDataComponentTypes.LETHAL_TEMPO_TIME_LEFT, lethalTempoComponent.keepTime());
            }
