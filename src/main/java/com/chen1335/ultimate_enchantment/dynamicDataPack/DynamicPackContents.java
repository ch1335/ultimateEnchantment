...
public class DynamicPackContents {
    ...
    /**
     * 以 {@link JsonElement} 形式存入数据。
     *
     * <p>与字节版共存而非替换：JSON 在<b>读取时</b>才序列化，这样动态包内的数据
     * 始终保持为结构化的 JSON 对象，而不是一段不透明的字节流 ——
     * 便于调试（可直接查看内容）与后续可能的运行时改写。
     * 对 Minecraft 而言两者等价，最终都由同一个 {@link IoSupplier} 提供字节流。
     */
    public void addToData(Identifier location, JsonElement json) {
        addToData(location, () -> {
